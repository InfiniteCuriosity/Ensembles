...
# Example 1: The fully automated solution in the Ensembles package beat all 699 entries from college students by more than 90%. Let's test that claim (spoiler alert: it's true), and show how you can use the same processes for any numerical data.

Welcome to the numerical example of the Ensembles package. This vignette will walk you through each of the steps and features of the numerical function, allowing you to beat the best score in a Kaggle contest by 699 college students. The student results were the Spring term project for 49 teams with 699 entries. This solution using the Ensembles package will beat all submission, and best their best solution by more than 90% and only require one line of code. The student results will be our baseline against which we will compare our results.
...
![One line of code. I added a couple of line breaks to make it easier to read, but R looks at this as one line of code.](images/One_line_of_code.jpg){width="752"}
...
The result is a perfect match for the y_ensemble value in the ensemble: 14.1, 18.4, 22.2, 29.0, 43.1, 16.7.
...
5.  Correlation table of the ensemble. Note that we are using very strongly correlated predictors in this example. Many of the correlations are above 0.90. If you look at the final column, y_ensemble, it will be seen that many of the several have correlations above 0.95. That will be addressed in the next example. For our present example, the head of the correlation table is: ![](images/Ensemble_correlation.png){width="750"}
...
8.  40 models, accuracy for each model, and each sample![Accuracy plot](images/accuracy_plot.jpg){width="750"}
...
12. Summary report, sorted by root mean squared error. Note that the lowest root mean squared error in our example is 0.1281.![Numerical results, sorted by root mean squared error](images/numerical_results.jpg){width="750"}
...
Let's look at a quick example. In the ISLR package, there is a data set called Carseats. It has both numerical and categorical values. Let's look at the head of the data set:

```{r Head of carseats data set}
library(ISLR)
head(Carseats, n = 10)
```

The three categorical columns are ShelveLoc (the location of the item on the shelf: Good, Medium or Bad), Urban (Yes or No) and US (Yes or No).

Let's see if the categorical columns are factors or not:

```{r Test if the categorical columns in Carseats are factors}
str(Carseats)
```
...
You can also set the data set up ahead of time so that all columns are numerical.

Here is how the function is set up to measure Sales. Note that how_to_handle_strings is now set to 1, which converts the strings to numbers that the function can use:\
\
![How to handle strings](images/how_to_handle_strings.jpg)

Once that is done, the function completes all the same steps as in the Boston Housing example. In this case the best holdout RMSE is 0.0475. Given that the mean of the sales price is 7.4963, the error is 0.006349799. Since the sale prices are in dollars, this is a little over 6/10 of a penny:

```{r Calculating the error for the Sales value in the Carseats data set}
0.0476/7.4963
```

Here is the top five results. Four of the top five results are ensembles, including the top two results:

![Top five results for modeling the Sales price in the Carseats data set](images/top_five_results.jpg)
...
![Numerical ensemble highly correlated predictors (partial list)](images/Numberical_ensemble_highly_correlated_predictors-02.jpg)
...
\
![Remove ensemble correlations below 0.95](images/Remove_ensemble_correlations_below_0.95.jpg)

The resulting ensemble does not have any predictors above 0.95. Let's test that claim:\
\
![No ensemble correlation above 0.95](images/Numerical_no_correlation_above_95.jpg)

This results in a slight decrease in holdout RMSE, but not by a lot:\
\
![Table: No correlation in the ensembles above 0.95](images/table_no_correlation_above_095.jpg)
...
Let's start by doing the analysis on the Boston housing data set in the Ensembles package. Note that we are going to make "Y" when it asks if we have new [data:\\](data:\){.uri}\
![Do you have new data?](images/do_you_have_new_data.jpg)
...
![NewBoston data in the Ensembles package](images/NewBoston.jpg)\
\
When the function is run, the Console asks for the location of the new [data:\\](data:\){.uri} ![New Boston on my computer, using it for new data](images/NewBoston_location-01.jpg)

The most accurate result (lowest RMSE) this time is Ensemble BayesGLM. Here's the evidence:

![Analysis of Boston minus the first five rows](images/Analysis_of_new_boston-01.jpg)
...
\
![Ensemble BayesGLM predictions on NewBoston data](images/Ensemble_BayesGLM_predictions.jpg)\
\
...
![Top of all trained models](images/all_trained_models.jpg)
