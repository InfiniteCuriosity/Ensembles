...
classification <- function(data, colnum, numresamples, do_you_have_new_data = c("Y", "N"), how_to_handle_strings = c(0("No strings"), 1("Strings as factors")), save_all_trained_models = c("Y", "N"),
  ...
  Results <- data.frame(
    ...
    "Overfitting_Min" = round(c(
      adabag_overfitting_range[1], bagging_overfitting_range[1], bag_rf_overfitting_range[1], C50_overfitting_range[1], linear_overfitting_range[1],
      n_bayes_overfitting_range[1], pls_overfitting_range[1], pda_overfitting_range[1], rf_overfitting_range[1], ranger_overfitting_range[1],
      rda_overfitting_range[1], rpart_overfitting_range[1], svm_overfitting_range[1], tree_overfitting_range[1], min(xgb_overfitting),
      ensemble_adabag_overfitting_range[1], ensemble_bag_cart_overfitting_range[1], ensemble_bag_rf_overfitting_range[1], ensemble_C50_overfitting_range[1],
      ensemble_n_bayes_overfitting_range[1], ensemble_ranger_overfitting_range[1], ensemble_rf_overfitting_range[1], ensemble_rda_overfitting_range[1], ensemble_svm_overfitting_range[1], ensemble_tree_overfitting_range[1]
    ), 4),
    ...
    "Overfitting_Max" = round(c(
      adabag_overfitting_range[2], bagging_overfitting_range[2], bag_rf_overfitting_range[2], C50_overfitting_range[2], linear_overfitting_range[2],
      n_bayes_overfitting_range[2], pls_overfitting_range[2], pda_overfitting_range[2], rf_overfitting_range[2], ranger_overfitting_range[2],
      rda_overfitting_range[2], rpart_overfitting_range[2], svm_overfitting_range[2], tree_overfitting_range[2], max(xgb_overfitting),
      ensemble_adabag_overfitting_range[2], ensemble_bag_cart_overfitting_range[2], ensemble_bag_rf_overfitting_range[2], ensemble_C50_overfitting_range[2],
      ensemble_n_bayes_overfitting_range[2], ensemble_ranger_overfitting_range[2], ensemble_rf_overfitting_range[2], ensemble_rda_overfitting_range[2], ensemble_svm_overfitting_range[2], ensemble_tree_overfitting_range[2]
    ), 4),
  ...
  accuracy_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, desc(Mean_Holdout_Accuracy)), y = Mean_Holdout_Accuracy)) +
    ...
    ggplot2::labs(x = "Model", y = "Mean Holdout Accuracy", title = "Model accuracy, closer to one is better") +
    ggplot2::geom_text(aes(label = Mean_Holdout_Accuracy), vjust = -0.5, hjust = -0.5, angle = 90) +
    ggplot2::ylim(0, 1.25)

  overfitting_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, desc(Overfitting_Mean)), y = Overfitting_Mean)) +
    ggplot2::geom_col()+
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 1, hjust=1)) +
  ...
  duration_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, Duration), y = Duration)) +
    ...
    ggplot2::ylim(0, max(Results$Duration) + 5)
  ...
  summary_tables <- list(
    "ADABag" = adabag_table, "Bagging" = bagging_table, "Bagged Random Forest" = bag_rf_table, "C50" = C50_table,
    "Linear" = linear_table, "Naive Bayes" = n_bayes_table,
    ...
    "Ensemble ADABag" = ensemble_adabag_table, "Ensemble Bagged Cart" = ensemble_bag_cart_table,
  ...
  total_data <- data.frame(
    ...
    "overfitting_mean" = c(
      ...
      n_bayes_holdout_mean, pls_holdout_mean, pda_holdout_mean, rf_holdout_mean, ranger_holdout_mean, rda_holdout_mean, rpart_holdout_mean, svm_holdout_mean, tree_holdout_mean, xgb_holdout_mean,
      ensemble_adabag_holdout_mean, ensemble_bag_cart_holdout_mean, ensemble_bag_rf_holdout_mean, ensemble_C50_holdout_mean,
  ...
  ###### Overfitting visualizations start here ########
  ...
  overfitting_plot <- ggplot2::ggplot(data = overfitting_data, mapping = ggplot2::aes(x = count, y = overfitting_mean, color = model)) +
    ggplot2::geom_line(mapping = ggplot2::aes(x = count, y = overfitting_mean)) +
    ggplot2::geom_point(mapping = ggplot2::aes(x = count, y = overfitting_mean)) +
    ggplot2::geom_hline(ggplot2::aes(yintercept = 1, color = "red")) +
    ggplot2::facet_wrap(~model, ncol = 5) +
    ggplot2::ggtitle("Overfitting by model, higher is better, 1 is best. \n the red horizontal line is 1.") +
    ggplot2::labs(y = "Overfitting by model, higher is better, 1 is best. \n the red line is 1.") +
    ggplot2::theme(legend.position = "none")
  ...
  if (do_you_have_new_data == "Y") {
    ...
    str(df)
    return(list(
      "Final_Results" = Final_results, "Barcharts" = barchart, "Accuracy_Barchart" = accuracy_barchart, "Duration_Barchart" = duration_barchart, "Overfitting_Barchart" = overfitting_barchart,
      "Data summary" = data_summary, "Correlation_Matrix" = correlation_marix, "Boxplots" = boxplots, "Histograms" = histograms, "Head of Ensembles" = head_ensemble,
      "Summary_Tables" = summary_tables, "Accuracy_Plot" = accuracy_plot, "Overfitting_plot" = overfitting_plot, "Total_Plot" = total_plot, "New_Data_Results" = New_Data_Results
    ))
  ...
  str(df)
  return(list(
    'Final_results' = Final_results, 'Barchart' = barchart, "Accuracy_Barchart" = accuracy_barchart, "Duration_Barchart" = duration_barchart, "Overfitting_Barchart" = overfitting_barchart,
    'Data_summary' = data_summary, 'Correlation_matrix' = correlation_marix,
    'Boxplots' = boxplots, 'Histograms' = histograms, 'Head_of_ensemble' = head_ensemble,
    'Summary_tables' = summary_tables, 'Accuracy_plot' = accuracy_plot, "Overfitting_plot" = overfitting_plot, 'Total_plot' = total_plot
  ))
}
