...
robust_duration <- 0
robust_duration_mean <- 0
robust_holdout_mean <- 0
robust_holdout_RMSE <- 0
robust_holdout_RMSE_mean <- 0
...
ensemble_neuralnet_test_RMSE_mean <- 0
ensemble_neuralnet_validation_RMSE <- 0
ensemble_neuralnet_validation_RMSE_mean <- 0
ensemble_neuralnet_test_predict_value <- 0
ensemble_neuralnet_RMSE_sd <- 0
ensemble_neuralnet_test_sd <- 0
ensemble_neuralnet_test_sd_df <- data.frame(neuralnet_test_sd)
ensemble_neuralnet_test_predict_value_mean <- 0
ensemble_y_hat_neuralnet <- 0
ensemble_neuralnet_sd <- 0
ensemble_neuralnet_sd_mean <- 0
ensemble_neuralnet_RMSE_difference <- 0
ensemble_neuralnet_train_RMSE <- 0
ensemble_neuralnet_train_RMSE_mean <- 0
ensemble_neuralnet_overfitting <- 0
ensemble_neuralnet_overfitting_mean <- 0
ensemble_neuralnet_holdout_RMSE <- 0
ensemble_neuralnet_holdout_RMSE_mean <- 0
ensemble_neuralnet_duration <- 0
ensemble_predict_new_data_nn <- 0
...
ensemble_pls_holdout_RMSE <- 0
ensemble_pls_holdout_RMSE_mean <- 0
ensemble_pls_predict_value_mean <- 0
...
ensemble_pcr_holdout_RMSE <- 0
ensemble_pcr_holdout_RMSE_mean <- 0
ensemble_pcr_predict_value_mean <- 0
...
for (i in 1:numresamples) {
  ...
  bag_rf_train_RMSE[i] <- Metrics::rmse(actual = train$y, predicted = predict(object = bag_rf_train_fit$best.model,
                                                                              newdata = train))
  bag_rf_train_RMSE_mean <- mean(bag_rf_train_RMSE)
  bag_rf_test_RMSE[i] <- Metrics::rmse(actual = test$y, predicted = predict(object = bag_rf_train_fit$best.model,
                                                                            newdata = test))
  bag_rf_test_RMSE_mean <- mean(bag_rf_test_RMSE)
  bag_rf_validation_RMSE[i] <- Metrics::rmse(actual = validation$y, predicted = predict(object = bag_rf_train_fit$best.model,
                                                                                        newdata = validation))
  ...
  boost_rf_train_RMSE[i] <- Metrics::rmse(actual = train$y, predicted = predict(object = boost_rf_train_fit$best.model,
                                                                                newdata = train))
  boost_rf_train_RMSE_mean <- mean(boost_rf_train_RMSE)
  boost_rf_test_RMSE[i] <- Metrics::rmse(actual = test$y, predicted = predict(object = boost_rf_train_fit$best.model,
                                                                              newdata = test))
  boost_rf_test_RMSE_mean <- mean(boost_rf_test_RMSE)
  boost_rf_validation_RMSE[i] = Metrics::rmse(actual = validation$y, predicted = predict(object = boost_rf_train_fit$best.model,
                                                                                         newdata = validation))
  ...
  knn_validation_RMSE[i] = Metrics::rmse(actual = validation$y, predicted = predict(object = knn_train_fit$best.model,
  ...
  lqs_validation_RMSE[i] = Metrics::rmse(actual = validation$y, predicted = predict(object = lqs_train_fit, newdata = validation))
  lqs_validation_RMSE_mean <- mean(lqs_validation_RMSE)
  lqs_holdout_RMSE[i] <- mean(c(lqs_test_RMSE_mean, lqs_validation_RMSE_mean))
  lqs_holdout_RMSE_mean <- mean(lqs_holdout_RMSE)
  ...
  robust_validation_RMSE[i] = Metrics::rmse(actual = validation$y, predicted = predict(object = MASS::rlm(y ~ .,data = train),
                                                                                       newdata = validation))
  robust_validation_RMSE_mean <- mean(robust_validation_RMSE)
  robust_holdout_RMSE[i]<- mean(c(robust_test_RMSE_mean, robust_validation_RMSE_mean))
  robust_holdout_RMSE_mean <- mean(robust_holdout_RMSE)
  robust_holdout_RMSE_sd_mean <- sd(c(robust_test_RMSE_mean, robust_validation_RMSE_mean))
  robust_train_predict_value <- as.numeric(predict(object = MASS::rlm(y ~ .,data = train), newdata = train))
  robust_test_predict_value <- as.numeric(predict(object = MASS::rlm(y ~ .,data = train), newdata = test))
  robust_validation_predict_value <- as.numeric(predict(object = MASS::rlm(y ~ .,data = train), newdata = validation))
  ...
  ensemble <- data.frame('BagRF' = y_hat_bag_rf* 1/bag_rf_holdout_RMSE_mean,
                         ...
                         'LQS' = y_hat_lqs*1/lqs_holdout_RMSE_mean,
...
bag_rf_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_bag_rf),
...
bagging_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_bagging),
...
bayesglm_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_bayesglm),
...
bayesrnn_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_bayesrnn),
...
boost_rf_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_boost_rf),
...
cubist_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_cubist),
...
elastic_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_elastic),
...
gam_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_gam),
...
gb_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_gb),
...
knn_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_knn),
...
lasso_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_lasso),
...
linear_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_linear),
...
#### LQS visualizations ####
lqs_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_lqs),
                     residuals = c(test$y, validation$y) - y_hat_lqs)

lqs_pred_vs_actual <- ggplot2::ggplot(lqs_df, mapping = aes(x = actual, y = as.numeric(y_hat_lqs))) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(intercept = 0, slope = 1, color="red") +
  ggplot2::labs(title = "LQS model: Predicted vs actual", x = "Actual", y = "Predicted")

lqs_resid_vs_actual <- ggplot2::ggplot(lqs_df, mapping = aes(x = actual, y = residuals)) +
  ggplot2::geom_point() +
  ggplot2::geom_hline(yintercept = 0, color="red") +
  ggplot2::labs(title = "LQS model: Residuals", x = "Actual", y = "Predicted")

lqs_hist_residuals <- ggplot2::ggplot(lqs_df, mapping = aes(x = residuals)) +
  ggplot2::geom_histogram(bins = round(nrow(df)/10)) +
  ggplot2::geom_vline(xintercept = 0, color="red") +
  ggplot2::labs(title = "LQS model: Histogram of residuals, each bar = 10 rows of data")

#### Neuralnet data visualizations ####
neuralnet_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_neuralnet),
...
pls_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_pls),
...
pcr_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_pcr),
...
rf_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_rf),
...
ridge_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_ridge),
...
robust_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_robust),
                        residuals = c(test$y, validation$y) - y_hat_robust)

robust_pred_vs_actual <- ggplot2::ggplot(robust_df, mapping = aes(x = actual, y = as.numeric(y_hat_robust))) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(intercept = 0, slope = 1, color="red") +
  ggplot2::labs(title = "Random Forest model: Predicted vs actual", x = "Actual", y = "Predicted")

robust_resid_vs_actual <- ggplot2::ggplot(robust_df, mapping = aes(x = actual, y = residuals)) +
  ggplot2::geom_point() +
  ggplot2::geom_hline(yintercept = 0, color="red") +
  ggplot2::labs(title = "Random Forest model: Residuals", x = "Actual", y = "Predicted")

robust_hist_residuals <- ggplot2::ggplot(robust_df, mapping = aes(x = residuals)) +
  ggplot2::geom_histogram(bins = round(nrow(df)/10)) +
  ggplot2::geom_vline(xintercept = 0, color="red") +
  ggplot2::labs(title = "Random Forest model: Histogram of residuals, each bar = 10 rows of data")

#### Rpart visualizations ####
rpart_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_rpart),
...
svm_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_svm),
...
#### Tree visualizations ####
tree_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_tree),
...
xgb_df <- data.frame(actual = c(test$y, validation$y), predicted = as.numeric(y_hat_xgb),
...
ensemble_bayesrnn_df <- data.frame(actual = c(ensemble_test$y_ensemble, ensemble_validation$y_ensemble),
...
ensemble_elastic_df <- data.frame(actual = c(ensemble_test$y, ensemble_validation$y), predicted = as.numeric(y_hat_ensemble_elastic),
...
ensemble_gb_df <- data.frame(actual = c(ensemble_test$y_ensemble, ensemble_validation$y_ensemble),
...
ensemble_knn_df <- data.frame(actual = c(ensemble_test$y, ensemble_validation$y), predicted = as.numeric(ensemble_y_hat_knn),
...
ensemble_lasso_df <- data.frame(actual = c(ensemble_test$y, ensemble_validation$y), predicted = as.numeric(y_hat_ensemble_lasso),
...
ensemble_rf_df <- data.frame(actual = c(ensemble_test$y_ensemble, ensemble_validation$y_ensemble),
...
ensemble_ridge_df <- data.frame(actual = c(ensemble_test$y, ensemble_validation$y), predicted = as.numeric(y_hat_ensemble_ridge),
...
ensemble_svm_df <- data.frame(actual = c(ensemble_test$y_ensemble, ensemble_validation$y_ensemble),
...
ensemble_xgb_df <- data.frame(actual = c(ensemble_test$y, ensemble_validation$y), predicted = as.numeric(ensemble_y_hat_xgb),
...
summary_results <- data.frame(
  ...
  'Model' = c('Actual data', 'Bagged Random Forest', 'Bagging', 'BayesGLM', 'BayesRNN',
     'BoostRF', 'Cubist', 'Elastic', 'GAM', 'Gradient Boosted', 'KNN', 'Lasso', 'Linear', 'LQS', 'Neuralnet', 'PLS',
     'PCR', 'RF', 'Ridge', 'Robust', 'Rpart', 'SVM', 'Tree', 'XGBoost', 'Ensemble Bagged Random Forest',
     ...
     'Ensemble Boosted RF', 'Ensemble Cubist','Ensemble RF', 'Ensemble Ridge', 'Ensemble Rpart',
     'Ensemble SVM', 'Ensemble Trees', 'Ensemble XGBoost'),

  'Mean_holdout_RMSE' = round(c(actual_RMSE, bag_rf_holdout_RMSE_mean, bagging_holdout_RMSE_mean, bayesglm_holdout_RMSE_mean,
     bayesrnn_holdout_RMSE_mean, boost_rf_holdout_RMSE_mean, cubist_holdout_RMSE_mean, elastic_holdout_RMSE_mean,
     gam_holdout_RMSE_mean, gb_holdout_RMSE_mean, knn_holdout_RMSE_mean, lasso_holdout_RMSE_mean, linear_holdout_RMSE_mean,
     lqs_holdout_RMSE_mean, neuralnet_holdout_RMSE_mean, pls_holdout_RMSE_mean, pcr_holdout_RMSE_mean, rf_holdout_RMSE_mean,
     ridge_holdout_RMSE_mean, robust_holdout_RMSE_mean, rpart_holdout_RMSE_mean, svm_holdout_RMSE_mean, tree_holdout_RMSE_mean, xgb_holdout_RMSE_mean,
     ...
     ensemble_bayesrnn_holdout_RMSE_mean, ensemble_boost_rf_holdout_RMSE_mean, ensemble_cubist_holdout_RMSE_mean,
     ensemble_rf_holdout_RMSE_mean, ensemble_ridge_holdout_RMSE_mean, ensemble_rpart_holdout_RMSE_mean,
     ensemble_svm_holdout_RMSE_mean, ensemble_tree_holdout_RMSE_mean, ensemble_xgb_holdout_RMSE_mean), 4),

  'Std_Deviation_of_holdout_RMSE' = round(c(actual_RMSE, bag_rf_holdout_RMSE_sd_mean, bagging_holdout_RMSE_sd_mean, bayesglm_holdout_RMSE_sd_mean,
     bayesrnn_holdout_RMSE_sd_mean, boost_rf_holdout_RMSE_sd_mean, cubist_holdout_RMSE_sd_mean, elastic_holdout_RMSE_sd_mean,
     ...
     lqs_holdout_RMSE_sd_mean, neuralnet_holdout_RMSE_sd_mean, pls_holdout_RMSE_sd_mean, pcr_holdout_RMSE_sd_mean, rf_holdout_RMSE_sd_mean,
     ridge_holdout_RMSE_sd_mean, robust_holdout_RMSE_sd_mean, rpart_holdout_RMSE_sd_mean, svm_holdout_RMSE_sd_mean, tree_holdout_RMSE_sd_mean, xgb_holdout_RMSE_sd_mean,
     ...
     ensemble_bayesrnn_holdout_RMSE_sd_mean, ensemble_boost_rf_holdout_RMSE_sd_mean, ensemble_cubist_holdout_RMSE_sd_mean,
     ensemble_rf_holdout_RMSE_sd_mean, ensemble_ridge_holdout_RMSE_sd_mean, ensemble_rpart_holdout_RMSE_sd_mean,
     ensemble_svm_holdout_RMSE_sd_mean, ensemble_tree_holdout_RMSE_sd_mean, ensemble_xgb_holdout_RMSE_sd_mean), 4),

  'Mean_data' = round(c(actual_mean, bag_rf_predict_value_mean, bagging_predict_value_mean, bayesglm_predict_value_mean,
     bayesrnn_predict_value_mean, boost_rf_predict_value_mean, cubist_predict_value_mean, elastic_test_predict_value_mean,
     gam_predict_value_mean, gb_predict_value_mean, knn_predict_value_mean, lasso_predict_value_mean, linear_predict_value_mean,
     lqs_predict_value_mean, neuralnet_test_predict_value_mean, pls_predict_value_mean, pcr_predict_value_mean, rf_predict_value_mean,
     ridge_test_predict_value_mean, robust_predict_value_mean, rpart_predict_value_mean, svm_predict_value_mean, tree_predict_value_mean, xgb_predict_value_mean,
     ...
     ensemble_boost_rf_predict_value_mean, ensemble_cubist_predict_value_mean, ensemble_rf_predict_value_mean,
     ensemble_ridge_predict_value_mean, ensemble_rpart_predict_value_mean, ensemble_svm_predict_value_mean,
     ensemble_tree_predict_value_mean, ensemble_xgb_predict_value_mean), 4),

  'Std_Dev_of_the_model' = round(c(actual_sd, bag_rf_sd_mean, bagging_sd_mean, bayesglm_sd_mean, bayesrnn_sd_mean,
     boost_rf_sd_mean, cubist_sd_mean, elastic_sd_mean, gam_sd_mean, gb_sd_mean, knn_sd_mean, lasso_sd_mean,
     linear_sd_mean, lqs_sd_mean, neuralnet_sd_mean, pls_sd_mean, pcr_sd_mean, rf_sd_mean, ridge_sd_mean,
     robust_sd_mean, rpart_sd_mean, svm_sd_mean, tree_sd_mean, xgb_sd_mean,
     ...
     ensemble_boost_rf_sd_mean, ensemble_cubist_sd_mean, ensemble_rf_sd_mean, ensemble_ridge_sd_mean,
     ensemble_rpart_sd_mean, ensemble_svm_sd_mean, ensemble_tree_sd_mean, ensemble_xgb_sd_mean), 4),
  ...
  'Mean_train_RMSE' = round(c(0, bag_rf_train_RMSE_mean, bagging_train_RMSE_mean, bayesglm_train_RMSE_mean, bayesrnn_train_RMSE_mean,
     boost_rf_train_RMSE_mean, cubist_train_RMSE_mean, elastic_train_RMSE_mean, gam_train_RMSE_mean, gb_train_RMSE_mean,
     knn_train_RMSE_mean, lasso_train_RMSE_mean, linear_train_RMSE_mean, lqs_train_RMSE_mean, neuralnet_train_RMSE_mean,
     pls_train_RMSE_mean, pcr_train_RMSE_mean, rf_train_RMSE_mean, ridge_train_RMSE_mean,
     robust_train_RMSE_mean, rpart_train_RMSE_mean, svm_train_RMSE_mean, tree_train_RMSE_mean, xgb_train_RMSE_mean,
     ...
     ensemble_boost_rf_train_RMSE_mean, ensemble_cubist_train_RMSE_mean,
     ensemble_rf_train_RMSE_mean, ensemble_ridge_train_RMSE_mean,
     ensemble_rpart_train_RMSE_mean, ensemble_svm_train_RMSE_mean,
     ensemble_tree_train_RMSE_mean, ensemble_xgb_train_RMSE_mean), 4),

  'Mean_test_RMSE' = round(c(0, bag_rf_test_RMSE_mean, bagging_test_RMSE_mean, bayesglm_test_RMSE_mean,
     bayesrnn_test_RMSE_mean, boost_rf_test_RMSE_mean, cubist_test_RMSE_mean, elastic_test_RMSE_mean,
     gam_test_RMSE_mean, gb_test_RMSE_mean, knn_test_RMSE_mean, lasso_test_RMSE_mean, linear_test_RMSE_mean,
     lqs_test_RMSE_mean, neuralnet_test_RMSE_mean, pls_test_RMSE_mean, pcr_test_RMSE_mean, rf_test_RMSE_mean, ridge_test_RMSE_mean,
     robust_test_RMSE_mean, rpart_test_RMSE_mean, svm_test_RMSE_mean, tree_test_RMSE_mean, xgb_test_RMSE_mean,
     ...
     ensemble_bayesrnn_test_RMSE_mean, ensemble_boost_rf_test_RMSE_mean, ensemble_cubist_test_RMSE_mean,
     ensemble_rf_test_RMSE_mean, ensemble_ridge_test_RMSE, ensemble_rpart_test_RMSE_mean, ensemble_svm_test_RMSE_mean,
     ensemble_tree_test_RMSE_mean, ensemble_xgb_test_RMSE_mean), 4),

  'Mean_validation_RMSE' = round(c(0, bag_rf_validation_RMSE_mean, bagging_validation_RMSE_mean, bayesglm_validation_RMSE_mean,
     bayesrnn_validation_RMSE_mean, boost_rf_validation_RMSE_mean, cubist_validation_RMSE_mean, elastic_validation_RMSE_mean,
     gam_validation_RMSE_mean, gb_validation_RMSE_mean, knn_validation_RMSE_mean, lasso_validation_RMSE_mean,
     linear_validation_RMSE_mean, lqs_validation_RMSE_mean, neuralnet_validation_RMSE_mean, pls_validation_RMSE_mean,
     pcr_validation_RMSE_mean, rf_validation_RMSE_mean, ridge_validation_RMSE_mean,
     robust_validation_RMSE_mean, rpart_validation_RMSE_mean, svm_validation_RMSE_mean, tree_validation_RMSE_mean, xgb_validation_RMSE_mean,
     ...
     ensemble_boost_rf_validation_RMSE_mean, ensemble_cubist_validation_RMSE_mean,
     ...
     ensemble_tree_validation_RMSE_mean, ensemble_xgb_validation_RMSE_mean), 4),

  'Overfitting' = round(c(0, bag_rf_overfitting_mean, bagging_overfitting_mean, bayesglm_overfitting_mean, bayesrnn_overfitting_mean,
     boost_rf_overfitting_mean, cubist_overfitting_mean, elastic_overfitting_mean, gam_overfitting_mean, gb_overfitting_mean,
     knn_overfitting_mean, lasso_overfitting_mean, linear_overfitting_mean, lqs_overfitting_mean, neuralnet_overfitting_mean,
     pls_overfitting_mean, pcr_overfitting_mean, rf_overfitting_mean, ridge_overfitting_mean,
     robust_overfitting_mean, rpart_overfitting_mean, svm_overfitting_mean, tree_overfitting_mean, xgb_overfitting_mean,
     ...
     ensemble_svm_overfitting_mean, ensemble_tree_overfitting_mean, ensemble_xgb_overfitting_mean), 4),

  'Duration' = round(c(0, bag_rf_duration_mean, bagging_duration_mean, bayesglm_duration_mean, bayesrnn_duration_mean,
     boost_rf_duration_mean, cubist_duration_mean, elastic_duration_mean, gam_duration_mean, gb_duration_mean,
     knn_duration_mean, lasso_duration_mean, linear_duration_mean, lqs_duration_mean, neuralnet_duration_mean,
     pls_duration_mean, pcr_duration_mean, rf_duration_mean, ridge_duration_mean,
     robust_duration_mean, rpart_duration_mean, svm_duration_mean, tree_duration_mean, xgb_duration_mean,
     ...
     ensemble_cubist_duration_mean, ensemble_rf_duration_mean, ensemble_ridge_duration_mean,
     ensemble_rpart_duration_mean, ensemble_svm_duration_mean, ensemble_tree_duration_mean, ensemble_xgb_duration_mean), 4)
)


summary_results <- summary_results %>% dplyr::arrange(Mean_holdout_RMSE)

final_results <- reactable::reactable(summary_results, searchable = TRUE, pagination = FALSE, wrap = TRUE, rownames = TRUE, fullWidth = TRUE, filterable = TRUE, bordered = TRUE,
...
if(data_visualizations[1] == 'Bagged Random Forest'){
  gridExtra::grid.arrange(bag_rf_pred_vs_actual, bag_rf_resid_vs_actual, bag_rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Bagging'){
  grid.arrange(bagging_pred_vs_actual, bagging_resid_vs_actual, bagging_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'BayesGLM'){
  grid.arrange(bayesglm_pred_vs_actual, bayesglm_resid_vs_actual, bayesglm_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'BayesRNN'){
  grid.arrange(bayesrnn_pred_vs_actual, bayesrnn_resid_vs_actual, bayesrnn_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Boost_rf'){
  grid.arrange(boost_rf_pred_vs_actual, boost_rf_resid_vs_actual, boost_rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Cubist'){
  grid.arrange(cubist_pred_vs_actual, cubist_resid_vs_actual, cubist_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Elastic'){
  grid.arrange(elastic_pred_vs_actual, elastic_resid_vs_actual, elastic_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'GAM'){
  grid.arrange(gam_pred_vs_actual, gam_resid_vs_actual, gam_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Gradient Boosted'){
  grid.arrange(gb_pred_vs_actual, gb_resid_vs_actual, gb_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'KNN'){
  grid.arrange(knn_pred_vs_actual, knn_resid_vs_actual, knn_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Lasso'){
  grid.arrange(lasso_pred_vs_actual, lasso_resid_vs_actual, lasso_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Linear'){
  gridExtra::grid.arrange(linear_pred_vs_actual, linear_resid_vs_actual,linear_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'LQS'){
  grid.arrange(lqs_pred_vs_actual, lqs_resid_vs_actual, lqs_hist_residuals, ncol = 2)
  gridExtra::grid.arrange(lqs_pred_vs_actual)
  gridExtra::grid.arrange(lqs_resid_vs_actual)
  gridExtra::grid.arrange(lqs_hist_residuals)
}

if(data_visualizations[1] == 'Neuralnet'){
  grid.arrange(neuralnet_pred_vs_actual, neuralnet_resid_vs_actual, neuralnet_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'PLS'){
  grid.arrange(pls_pred_vs_actual, pls_resid_vs_actual, pls_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'PCR'){
  grid.arrange(pcr_pred_vs_actual, pcr_resid_vs_actual, pcr_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'RF'){
  grid.arrange(rf_pred_vs_actual, rf_resid_vs_actual, rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ridge'){
  grid.arrange(ridge_pred_vs_actual, ridge_resid_vs_actual, ridge_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Robust'){
  grid.arrange(robust_pred_vs_actual, robust_resid_vs_actual, robust_hist_residuals, ncol = 2)
  gridExtra::grid.arrange(robust_pred_vs_actual)
  gridExtra::grid.arrange(robust_resid_vs_actual)
  gridExtra::grid.arrange(robust_hist_residuals)
}

if(data_visualizations[1] == 'Rpart'){
  grid.arrange(rpart_pred_vs_actual, rpart_resid_vs_actual, rpart_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'SVM'){
  grid.arrange(svm_pred_vs_actual, svm_resid_vs_actual, svm_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Tree'){
  grid.arrange(tree_pred_vs_actual, tree_resid_vs_actual, tree_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'XGBoost'){
  grid.arrange(xgb_pred_vs_actual, xgb_resid_vs_actual, xgb_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Bagged Random Forest'){
  gridExtra::grid.arrange(ensemble_bag_rf_pred_vs_actual, ensemble_bag_rf_resid_vs_actual, ensemble_bag_rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Bagging'){
  gridExtra::grid.arrange(ensemble_bagging_pred_vs_actual, ensemble_bagging_resid_vs_actual, ensemble_bagging_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble BayesGLM'){
  gridExtra::grid.arrange(ensemble_bayesglm_pred_vs_actual, ensemble_bayesglm_resid_vs_actual, ensemble_bayesglm_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble BayesRNN'){
  gridExtra::grid.arrange(ensemble_bayesrnn_pred_vs_actual, ensemble_bayesrnn_resid_vs_actual, ensemble_bayesrnn_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Boosted RF'){
  gridExtra::grid.arrange(ensemble_boost_rf_pred_vs_actual, ensemble_boost_rf_resid_vs_actual, ensemble_boost_rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Cubist'){
  gridExtra::grid.arrange(ensemble_cubist_pred_vs_actual, ensemble_cubist_resid_vs_actual, ensemble_cubist_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble_Elastic'){
  grid.arrange(ensemble_elastic_pred_vs_actual, ensemble_elastic_resid_vs_actual, ensemble_elastic_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Gradient Boosted'){
  gridExtra::grid.arrange(ensemble_gb_pred_vs_actual, ensemble_gb_resid_vs_actual, ensemble_gb_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble K-Nearest Neighbors'){
  grid.arrange(ensemble_knn_pred_vs_actual, ensemble_knn_resid_vs_actual, ensemble_knn_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble_Lasso'){
  grid.arrange(ensemble_lasso_pred_vs_actual, ensemble_lasso_resid_vs_actual, ensemble_lasso_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Linear'){
  gridExtra::grid.arrange(ensemble_linear_pred_vs_actual, ensemble_linear_resid_vs_actual, ensemble_linear_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble RF'){
  gridExtra::grid.arrange(ensemble_rf_pred_vs_actual, ensemble_rf_resid_vs_actual, ensemble_rf_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble_Ridge'){
  grid.arrange(ensemble_ridge_pred_vs_actual, ensemble_ridge_resid_vs_actual, ensemble_ridge_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Rpart'){
  gridExtra::grid.arrange(ensemble_rpart_pred_vs_actual, ensemble_rpart_resid_vs_actual, ensemble_rpart_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Support Vector Machines'){
  gridExtra::grid.arrange(ensemble_svm_pred_vs_actual, ensemble_svm_resid_vs_actual, ensemble_svm_hist_residuals, ncol = 2)
...
if(data_visualizations[1] == 'Ensemble Trees'){
  gridExtra::grid.arrange(ensemble_tree_pred_vs_actual, ensemble_tree_resid_vs_actual, ensemble_tree_hist_residuals, ncol = 2)
...
accuracy_data <-
  data.frame(
    ...
    'model' = c(rep('Bagged Random Forest', numresamples), c(rep('Bagging', numresamples)), c(rep('BayesGLM', numresamples)),
      c(rep('BayesRNN', numresamples)), c(rep('Boost Random Forest', numresamples)),
      c(rep('Cubist', numresamples)),  c(rep('Elastic', numresamples)), c(rep('Generalized Additive Models (GAM)', numresamples)),
      ...
      c(rep('LQS', numresamples)), c(rep('Neuralnet', numresamples)), c(rep('Principal Components Regression', numresamples)), c(rep('Partial Least Squares', numresamples)),
      c(rep('Random Forest', numresamples)), c(rep('Ridge', numresamples)), c(rep('Robust', numresamples)), c(rep('RPart', numresamples)),
      ...
      c(rep('Ensemble Boosted Random Forest', numresamples)), c(rep('Ensemble Cubist', numresamples)),
    ...
    'data' = c(bag_rf_holdout_RMSE, bagging_holdout_RMSE, bayesglm_holdout_RMSE,
      bayesrnn_holdout_RMSE, boost_rf_holdout_RMSE,
      cubist_holdout_RMSE, elastic_holdout_RMSE_df$elastic_holdout_RMSE[2:nrow(elastic_holdout_RMSE_df)], gam_holdout_RMSE,
      ...
      lqs_holdout_RMSE, neuralnet_holdout_RMSE, pcr_holdout_RMSE, pls_holdout_RMSE,
      rf_holdout_RMSE, ridge_holdout_RMSE_df$ridge_holdout_RMSE[2:nrow(ridge_holdout_RMSE_df)], robust_holdout_RMSE, rpart_holdout_RMSE,
      ...
      ensemble_boost_rf_holdout_RMSE, ensemble_cubist_holdout_RMSE,
    ...
    'mean' = rep(c(bag_rf_holdout_RMSE_mean,bagging_holdout_RMSE_mean, bayesglm_holdout_RMSE_mean,
      bayesrnn_holdout_RMSE_mean, boost_rf_test_RMSE_mean,
      cubist_holdout_RMSE_mean, elastic_holdout_RMSE_mean, gam_holdout_RMSE_mean,
      gb_holdout_RMSE_mean, knn_holdout_RMSE_mean, lasso_holdout_RMSE_mean, linear_holdout_RMSE_mean,
      lqs_holdout_RMSE_mean, neuralnet_holdout_RMSE_mean, pcr_holdout_RMSE_mean, pls_holdout_RMSE_mean,
      rf_holdout_RMSE_mean, ridge_holdout_RMSE_mean, robust_holdout_RMSE_mean, rpart_holdout_RMSE_mean,
      ...
      ensemble_tree_holdout_RMSE_mean, ensemble_xgb_holdout_RMSE_mean), each = numresamples)
  )
...
total_data <-
  data.frame(
    'count' = 1:numresamples,
    'model' =
      c(rep('Bagged Random Forest', numresamples), c(rep('Bagging', numresamples)), c(rep('BayesGLM', numresamples)),
      c(rep('BayesRNN', numresamples)), c(rep('Boost Random Forest', numresamples)),
      c(rep('Cubist', numresamples)),  c(rep('Elastic', numresamples)), c(rep('Generalized Additive Models (GAM)', numresamples)),
      c(rep('Gradient Boosted', numresamples)), c(rep('K-Nearest Neighbors', numresamples)), c(rep('Lasso', numresamples)), c(rep('Linear', numresamples)),
      c(rep('LQS', numresamples)), c(rep('Neuralnet', numresamples)), c(rep('Principal Components Regression', numresamples)), c(rep('Partial Least Squares', numresamples)),
    ...
    'train' = c(bag_rf_train_RMSE, bagging_train_RMSE, bayesglm_train_RMSE,
      bayesrnn_train_RMSE, boost_rf_train_RMSE,
      cubist_train_RMSE, elastic_train_RMSE_df$elastic_train_RMSE[2:nrow(elastic_train_RMSE_df)], gam_train_RMSE,
      gb_train_RMSE, knn_train_RMSE, lasso_train_RMSE_df$lasso_train_RMSE[2:nrow(lasso_train_RMSE_df)], linear_train_RMSE,
      lqs_train_RMSE, neuralnet_train_RMSE, pcr_train_RMSE, pls_train_RMSE,
      rf_train_RMSE, ridge_train_RMSE_df$ridge_train_RMSE[2:nrow(ridge_train_RMSE_df)], robust_train_RMSE, rpart_train_RMSE,
      ...
      ensemble_boost_rf_train_RMSE, ensemble_cubist_train_RMSE,
    ...
    'test' = c(bag_rf_test_RMSE, bagging_test_RMSE, bayesglm_test_RMSE,
      bayesrnn_test_RMSE, boost_rf_test_RMSE,
      cubist_test_RMSE, elastic_test_RMSE_df$elastic_test_RMSE[2:nrow(elastic_test_RMSE_df)], gam_test_RMSE,
      gb_test_RMSE, knn_test_RMSE, lasso_test_RMSE_df$lasso_test_RMSE[2:nrow(lasso_test_RMSE_df)], linear_test_RMSE,
      lqs_test_RMSE, neuralnet_test_RMSE, pcr_test_RMSE, pls_test_RMSE,
      rf_test_RMSE, ridge_test_RMSE_df$ridge_test_RMSE[2:nrow(ridge_test_RMSE_df)], robust_test_RMSE, rpart_test_RMSE,
      svm_test_RMSE, tree_test_RMSE, xgb_test_RMSE,
      ensemble_bag_rf_test_RMSE, ensemble_bagging_test_RMSE, ensemble_elastic_test_RMSE_df$ensemble_elastic_test_RMSE[2:nrow(ensemble_elastic_test_RMSE_df)],
      ensemble_bayesglm_test_RMSE, ensemble_bayesrnn_test_RMSE,
      ensemble_boost_rf_test_RMSE, ensemble_cubist_test_RMSE,
      ensemble_gb_test_RMSE, ensemble_knn_test_RMSE_df$ensemble_knn_test_RMSE[2:nrow(ensemble_knn_test_RMSE_df)],
      ensemble_lasso_test_RMSE_df$ensemble_lasso_test_RMSE[2:nrow(ensemble_lasso_test_RMSE_df)],
      ensemble_linear_test_RMSE, ensemble_rf_test_RMSE,
      ensemble_ridge_test_RMSE_df$ensemble_ridge_test_RMSE[2:nrow(ensemble_ridge_test_RMSE_df)],
      ensemble_rpart_test_RMSE, ensemble_svm_test_RMSE,
      ensemble_tree_test_RMSE, ensemble_xgb_test_RMSE),

    'validation' = c(bag_rf_validation_RMSE, bagging_validation_RMSE, bayesglm_validation_RMSE,
      bayesrnn_validation_RMSE, boost_rf_validation_RMSE,
      cubist_validation_RMSE, elastic_validation_RMSE_df$elastic_validation_RMSE[2:nrow(elastic_validation_RMSE_df)], gam_validation_RMSE,
      gb_validation_RMSE, knn_validation_RMSE, lasso_validation_RMSE_df$lasso_validation_RMSE[2:nrow(lasso_validation_RMSE_df)], linear_validation_RMSE,
      lqs_validation_RMSE, neuralnet_validation_RMSE, pcr_validation_RMSE, pls_validation_RMSE,
      rf_validation_RMSE, ridge_validation_RMSE_df$ridge_validation_RMSE[2:nrow(ridge_validation_RMSE_df)], robust_validation_RMSE, rpart_validation_RMSE,
      svm_validation_RMSE, tree_validation_RMSE, xgb_validation_RMSE,
      ensemble_bag_rf_validation_RMSE, ensemble_bagging_validation_RMSE, ensemble_elastic_validation_RMSE_df$ensemble_elastic_validation_RMSE[2:nrow(ensemble_elastic_validation_RMSE_df)],
      ensemble_bayesglm_validation_RMSE, ensemble_bayesrnn_validation_RMSE,
      ensemble_boost_rf_validation_RMSE, ensemble_cubist_validation_RMSE,
      ensemble_gb_validation_RMSE, ensemble_knn_validation_RMSE_df$ensemble_knn_validation_RMSE[2:nrow(ensemble_knn_validation_RMSE_df)],
      ensemble_lasso_validation_RMSE_df$ensemble_lasso_validation_RMSE[2:nrow(ensemble_lasso_validation_RMSE_df)],
      ensemble_linear_validation_RMSE, ensemble_rf_validation_RMSE,
      ensemble_ridge_validation_RMSE_df$ensemble_ridge_validation_RMSE[2:nrow(ensemble_ridge_validation_RMSE_df)],
      ensemble_rpart_validation_RMSE, ensemble_svm_validation_RMSE,
      ensemble_tree_validation_RMSE, ensemble_xgb_validation_RMSE),

    'holdout' = c(bag_rf_holdout_RMSE, bagging_holdout_RMSE, bayesglm_holdout_RMSE,
      bayesrnn_holdout_RMSE, boost_rf_holdout_RMSE,
      cubist_holdout_RMSE, elastic_holdout_RMSE_df$elastic_holdout_RMSE[2:nrow(elastic_holdout_RMSE_df)], gam_holdout_RMSE,
      gb_holdout_RMSE, knn_holdout_RMSE, lasso_holdout_RMSE_df$lasso_holdout_RMSE[2:nrow(lasso_holdout_RMSE_df)], linear_holdout_RMSE,
      lqs_holdout_RMSE, neuralnet_holdout_RMSE, pcr_holdout_RMSE, pls_holdout_RMSE,
...
total_plot <- ggplot2::ggplot(data = total_data, mapping = ggplot2::aes(x = count, y = data, color = model)) +
  ...
  ggplot2::scale_color_manual(name='Total Results',
                              ...
                              values=c('train' = 'blueviolet', 'test' = 'darkcyan', 'validation' = 'darkgray', 'holdout' = 'turquoise1', 'mean' = 'black',
                                       'optimal' = 'red'))
...
if(do_you_have_new_data == "Y"){
  ...
  new_lqs <- predict(object = lqs_train_fit, newdata = new_data)
  ...
  new_ensemble <- data.frame(
    ...
    'LQS' = new_lqs/lqs_holdout_RMSE_mean,
  ...
  if(remove_ensemble_correlations_greater_than >0){
    ...
    new_ensemble <- select(new_ensemble, thing1)
  }
  ...
  predictions_of_new_data <- reactable::reactable(data = df1, searchable = TRUE, pagination = FALSE, wrap = TRUE, rownames = TRUE, fullWidth = TRUE, filterable = TRUE, bordered = TRUE,
                                                  striped = TRUE, highlight = TRUE,  resizable = TRUE) %>%
  ...
  if(save_all_trained_models == "Y"){
    ...
    lqs_train_fit <<- lqs_train_fit # LQS models
    ...
    robust_train_fit <<-robust_train_fit # Robust models
  ...
  str(df)
  return(list('accuracy_plot' = accuracy_plot, 'total_plot' = total_plot, 'histograms' = histograms, 'boxplots' = boxplots, 'final_results_table' = final_results,
